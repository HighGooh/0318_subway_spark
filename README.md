...
## 🔨 트러블 슈팅 레포트
### 1. 데이터 처리 공정 최적화를 통한 시스템 성능 개선
- **Problem**: 별도의 적재(Staging) 테이블 없이 실시간 데이터를 직접 처리함에 따라, 대용량 데이터 조회 및 가공 시 시스템 부하 급증 및 처리 속도 저하 문제 발생.

- **Solution**: **Apache Spark(PySpark)**를 도입하여 데이터 파이프라인 최적화. 복잡한 연산이 필요한 데이터를 사전에 가공하여 View 객체로 구조화함으로써 DB 서버의 직접적인 과부하를 방지하고 연산 효율을 높임.

- **Result**: 기존 방식 대비 데이터 처리 속도를 약 75% 단축시키는 획기적인 성능 개선 달성.


### 2. LLM 프롬프트 엔지니어링을 통한 AI 에이전트 응답 정확도 향상
- **Problem** n8n 기반 AI 에이전트 서비스 개발 중, 모델 성능에 따라 응답 값이 일관되지 않거나 비정형 데이터가 반환되어 후속 워크플로우 실행에 오류가 발생하는 이슈 발생.

- **Solution** 프롬프트 엔지니어링(Prompt Engineering) 기법 적용. 고정 시스템 프롬프트를 설정하고, 출력 형식(JSON 등)과 자료형(Type)을 명확히 정의하여 모델의 추론 방향을 제어.

- **Result** AI 에이전트와 데이터베이스 간의 안정적인 데이터 연동 성공. 이를 통해 AI 서비스의 기술적 메커니즘에 대한 깊은 이해와 워크플로우 자동화 구현 경험 확보.
...
## 📌 Project Overview
출퇴근 시간대의 승하차 인원을 분석하여, 지역 별 성격을 주거지역과 업무지역으로 나누어 사용자에게 직관적인 시각화 데이터를 제공하는 풀스택 웹 애플리케이션. 2008년~2024년의 데이터를 정제하고 분석한 결과값을 도출해 냅니다.
...
> **2008년~2024년 어린이날 하차 데이터를 활용한 나들이 트렌드 시각화 프로젝트**

## 📌 Project Overview
어린이날 가족 나들이객의 하차 인원을 분석하여, 연도별 혼잡 거점의 변화를 파악하고 사용자에게 직관적인 시각화 데이터를 제공

## 🛠 Tech Stack
- **Frontend**: React, Recharts(Bar, Line Chart), Axios, Bootstrap
...
- **데이터 증거**: 
 ...
 2. 현재 (~2024년): 시간이 지날수록 **홍대입구역(젊은 층의 문화 소비)**과 **고속터미널역(쇼핑 및 복합 쇼핑몰)**의 하차 비율이 가파르게 상승하며 어린이대공원을 추월하거나 대등한 수준에 도달함.
...
- **Result** 불필요한 API 호출을 차단하고, 데이터 로딩 중 시스템 안정성을 확보하여 매끄러운 사용자 경험 제공.
